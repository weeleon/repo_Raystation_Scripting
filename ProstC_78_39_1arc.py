...
# Set up a standard VMAT protocol plan with 1 counter clockwise arc
with CompositeAction('Adding VMAT arc/s for name {0} '.format(planName)):
    # add plan
    plan = patient.AddNewPlan(PlanName=planName, Comment="Prostate VMAT auto-plan", ExaminationName=examinationName)
...
# Create prescription, clinical goals and optimisation functions for the first VMAT beamset
with CompositeAction('Add prescription, beams, clinical goals, optimization functions'):
	...
	# add two partial arc vmat fields around the ROI-based isocenter
	beamSet1.CreateArcBeam(
		Name = '1; 180_CC_0',
		Energy=defaultPhotonEn,
		CouchAngle = 0,
		GantryAngle = 179.9,
		CollimatorAngle = 30,
		ArcStopGantryAngle = 0.0,
		ArcDirectionRotation = 'CounterClockwise'
		Isocenter = {'x':isocenter.x, 'y':isocenter.y, 'z':isocenter.z}
		)
	beamSet2.CreateArcBeam(
		Name = '2; 180_CW_0',
		Energy=defaultPhotonEn,
		CouchAngle = 0,
		GantryAngle = 180.1,
		CollimatorAngle = 330,
		ArcStopGantryAngle = 0.0,
		ArcDirectionRotation = 'Clockwise'
		Isocenter = {'x':isocenter.x, 'y':isocenter.y, 'z':isocenter.z}
		)
